...
test('capture evidence, cluster into an insight, promote to an opportunity, promote to an initiative', async ({
  ...
  await page.getByLabel('Evidence content').fill(evidenceContent);
  ...
  await page.getByLabel('Title').fill(insightTitle);
  await page.getByLabel('Summary').fill('Summary of the E2E insight.');
  await page.getByText(evidenceContent).click(); // the evidence checkbox
  await page.getByRole('button', { name: 'Add insight' }).click();
  await expect(page.getByRole('cell', { name: insightTitle })).toBeVisible();
  ...
  await page.getByLabel('Problem framing').fill('Framing for the E2E opportunity.');
  await page.getByText(insightTitle).click(); // the insight checkbox
